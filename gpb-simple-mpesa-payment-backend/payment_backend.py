...
class Settings(BaseSettings):
    ...
    openapi_title: str = "GPB Simple Mpesa Payment Backend"
    ...
    async def disburse_loop(self):
        while True:
            ...
            async with async_session_maker() as session:
                ...
                if db_response:
                    _logger.info(
                        "GPB Simple Mpesa - processing payment from payment list."
                    )
                    await self.disburse(db_response, session)
                else:
                    _logger.info(
                        "GPB Simple Mpesa - no records found in payment list table."
                    )
